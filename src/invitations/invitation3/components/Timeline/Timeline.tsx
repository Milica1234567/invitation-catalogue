...
function Timeline() {
  return (
    <section className="inv3-timeline" aria-labelledby="inv3-timeline-title">
      <div className="inv3-content">
        <header data-inv3-reveal>
          <p>Redosled svečanosti</p>
          <h2 id="inv3-timeline-title">Plan dana</h2>
        </header>
        <div className="inv3-timeline__items">
          {timelineItems.map((item, index) => (
            <article data-inv3-reveal style={{ "--inv3-delay": `${index * 0.12}s` } as CSSProperties} key={item.time}>
              <span className={`inv3-timeline__mark inv3-timeline__mark--${item.mark}`} aria-hidden="true"><i /><i /><i /></span>
              <time>{item.time}</time>
              <h3>{item.title}</h3>
              <p>{item.place}</p>
            </article>
