...
function Invitation7() {
  ...
  useEffect(() => {
    const elements = document.querySelectorAll<HTMLElement>(".invitation-seven [data-inv7-reveal]");
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (!entry.isIntersecting) return;
        entry.target.classList.add("inv7-visible");
  ...
  return (
    <>
      ...
      <main className={`invitation-seven ${revealed ? "invitation-seven--revealed" : ""}`}>
        <Hero />
        <PhotoIntro />
        <InvitationMessage />
        <WeddingDetails />
